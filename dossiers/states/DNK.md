---
id: ECL-STATE-DNK
entity: "Denmark"
iso3: DNK
issue: 46
provisional_outcome: S
provisional_scope: "specified Udbetaling Danmark / ATP automated fraud-risk profiling systems and materially connected workflows that remain unremediated"
confidence: high
evidence_cutoff: 2026-08-11
last_reviewed: 2026-08-11
review_stage: scoped-adversarial-reviewed
adversarial_result: "UPHOLD S; system-specific narrowing"
operative: false
---
# Denmark

> Governance record only; no licensing effect by itself.

## 1. Current determination

**S — Restricted with defined scope**, limited to particular Udbetaling Danmark/ATP automated fraud/risk-profiling models and materially connected data-processing workflows where discriminatory, opaque or disproportionate surveillance effects remain unremediated. ATP, Udbetaling Danmark and Danish welfare administration are **not** restricted generally.

## 2. ECL criteria engaged

Repressive/disproportionate data surveillance, discriminatory profiling, opacity/non-contestability and reduction of meaningful agency in welfare administration.

## 3. Evidence supporting the determination

Amnesty's technical investigation documents dozens of data-driven fraud-control algorithms used to flag welfare recipients for investigation, including models using foreign affiliation/citizenship and atypical household patterns, with mass-data processing, opacity and discrimination risks. The authorities declined requests for collaborative audit/full code and data access, and the review did not locate authoritative evidence that the implicated architecture had been terminated or independently cleared by August 2026.

## 4. Counter-evidence and exergic institutions

Danish authorities dispute parts of Amnesty's legal/technical characterization, including whether the systems constitute prohibited social scoring under the EU AI Act. Denmark also has ordinary judicial/administrative review and EU data-protection/AI-law safeguards, weighing strongly against any broader State designation.

## 5. Adversarial determination

**UPHOLD S, NARROW TO SPECIFIED SYSTEMS.**

## 6. Removal trigger

Independently auditable retirement/redesign, validated non-discrimination/proportionality assessment, adequate notice/contestability and effective remediation for affected persons.

## 7. Sources

- https://www.amnesty.org/en/documents/eur18/8709/2024/en/
- https://www.amnesty.org/en/documents/eur18/9419/2025/en/
- https://www.atp.dk/om-os/organisationen
- https://www.atp.dk/karriere/udbetaling-danmark

## 8. Historical evidence absorption

Historical source: `../../reviews/2026/adversarial/scoped/tranche-01.md` (cutoff 2026-08-11). Later consistency controls: `../../reviews/2026/consistency/ecl-0.2-state-delta.md` and `../../reviews/2026/consistency/schedule-readiness.md`. Issue #46 contains no additional public evidence comment.

- **Status:** absorbed / already represented. The initial adjudication already classified Denmark as `S`; scoped tranche 1 upheld the tier while making the controlling attribution correction explicit: the finding follows specified Udbetaling Danmark/ATP fraud-risk profiling systems and materially connected workflows, not ATP generally, Udbetaling Danmark generally or Danish welfare administration generally. The later ECL 0.2 State Delta retained `S`.
- The ATP organization and Udbetaling Danmark locators above come from the later Schedule translation and identify the institutional/workflow boundary. They are **identity/context provenance**, not proposition-specific proof that prohibited profiling occurred; the Amnesty technical records remain the conduct-evidence baseline.
- **Schedule boundary:** `../../registry/schedule-state-s-freezes/promoted-ready-01.yml` controls current renderability. It freezes only specified Udbetaling Danmark automated fraud/risk-profiling models in qualifying discriminatory, opaque or disproportionate welfare-profiling capacities. ATP pension/investment activity, ordinary welfare-payment administration and systems independently retired, corrected and validated are excluded.

## 9. Procedural history

Issue #46 · `../../reviews/2026/initial-adjudication.md` · `../../reviews/2026/adversarial/scoped/tranche-01.md` · `../../reviews/2026/consistency/ecl-0.2-state-delta.md` · `../../reviews/2026/consistency/schedule-readiness.md`
