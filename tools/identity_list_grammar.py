...
# ``including``, ``plus``, ``together with``, and ``alongside`` must behave like the other
# explicit identity-list coordinators so a trailing member cannot disappear from State person
# or Schedule capacity-list completeness checks.
WORD_COORDINATOR_PATTERN = r"(?:and\s*/\s*or|and-or|and/or|as\s+well\s+as|together\s+with|alongside|including|plus|and|or)"
