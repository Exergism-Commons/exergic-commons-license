...
def _validate_bundle_manifest_shape(bundle: Any) -> dict[str, Any]:
    ...
    if bundle["operative"]:
        review = bundle.get("legal_review")
        if not isinstance(review, dict):
            raise ValueError("operative bundle manifest requires immutable legal_review metadata")
        if set(review) != {"ref", "path", "sha256"}:
            raise ValueError("bundle legal_review must contain exactly ref, path and sha256")
        if not isinstance(review.get("ref"), str) or not review["ref"]:
            raise ValueError("bundle legal_review ref must be a non-empty string")
        _validate_metadata_path(review.get("path"), label="bundle legal_review")
        _validate_sha256(review.get("sha256"), label="bundle legal_review sha256")
...
def _safe_bundle_manifest_path(repo_root: Path, bundle_ref: str) -> Path:
    ...
    return repo_root / "releases" / "bundles" / f"{bundle_ref}.json"
...
def build_distribution(
    ...
    *,
    ...
    if manifest_path.is_symlink() or not manifest_path.is_file():
        raise ValueError(f"missing immutable Bundle manifest: {manifest_path}")
