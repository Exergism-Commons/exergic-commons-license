...
A `supersedes` reference is graph integrity, not free text: a Claim may supersede only a resolving Claim identity and an EvidenceItem may supersede only a resolving EvidenceItem identity. A mistyped or cross-kind target fails SHACL/SPARQL integrity.
...
The automatic accepted-Claim conflict guard applies only to predicates explicitly curated as mutually exclusive/single-valued in the integrity policy. It must **not** treat ordinary multi-valued relationships such as `tracks`, `controls` or `deploys` as contradictory merely because they have different objects. Literal conflict comparison is RDF-term-sensitive: datatype-distinct values such as integer `1` and string `"1"` remain distinct.
...
No Claim may assert `ecl:outcome` or a governance/tier/restriction-status predicate as its proposition. Governance-like predicate names are compared after separator normalization so spelling variants cannot bypass the guard. No Claim object may be a `GovernanceOutcome`. Governance remains in the reviewed dossier / GovernanceDecision process, and only an exact incorporated Schedule has licensing effect.
